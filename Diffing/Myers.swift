...
func _myers<E>(
  ...
  func _descent(
    ...
    let offX = a.range.startIndex
    let offY = b.range.startIndex
    let n = a.range.endIndex - a.range.startIndex
    let m = b.range.endIndex - b.range.startIndex
    ...
    iterator: for d in 0...(n + m) {
      ...
      for k in stride(from: -d, through: d, by: 2) {
        ...
        while x < n && y < m {
          if !cmp(a.base[offX + x], b.base[offY + y]) {
            break;
          }
          x &+= 1
          y &+= 1
  ...
  func _formChanges(
    ...
    for d in stride(from: trace.count &- 1, to: 0, by: -1) {
      ...
      if y != prev_y {
        ...
        let actualIndex = a.range.startIndex + prev_x
        changes.append(.remove(offset: actualIndex, element: a.base[actualIndex], associatedWith: nil))
      }
